  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#ffffff",fontFamily:"'Barlow',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow:ital,wght@0,400;0,700;0,900;1,700;1,900&family=Barlow+Condensed:ital,wght@0,400;0,700;0,900;1,700;1,900&display=swap');
        ...
        .lbt{width:100%;margin-top:12px;background:#E8481C;color:#fff;border:none;padding:13px;font-family:'Barlow',sans-serif;font-size:13px;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;border-radius:2px;}
        @keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-8px)}75%{transform:translateX(8px)}}
        .shake{animation:shake .3s;}`}</style>
      <div className={`lb${shake?" shake":""}`}>
        <div style={{fontFamily:"'Barlow',sans-serif",fontSize:"28px",fontWeight:"900",fontStyle:"italic",textTransform:"uppercase",color:"#fff",marginBottom:"4px"}}>PROF G</div>
        <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".15em",textTransform:"uppercase",marginBottom:"36px"}}>Intelligence Dashboard</div>
...
function TakeawayBlock({showName, color, ytVideos, fallback}) {
  ...
  return (
    <div style={{background:"#f5f5f5",border:`1px solid ${color}33`,borderRadius:"2px",padding:"20px 24px",marginBottom:"12px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"14px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          {data&&<span style={{fontSize:"12px",color:"#aaaaaa"}}>({data.period} · {data.metric})</span>}
        </div>
        <button onClick={run} disabled={loading} style={{background:"transparent",border:`1px solid ${color}55`,color,padding:"5px 14px",fontSize:"14px",letterSpacing:".08em",textTransform:"uppercase",cursor:"pointer",fontFamily:"'Barlow',sans-serif",borderRadius:"2px",opacity:loading?0.5:1}}>
          {loading?"Analyzing…":data?"Refresh":"Generate with AI"}
        </button>
      </div>
      {!hasYT&&!data&&<div style={{fontSize:"14px",color:"#aaaaaa",marginBottom:"8px"}}>⚠ Using spreadsheet data — YouTube loading in background for fresher results</div>}
      {hasYT&&!data&&<div style={{fontSize:"14px",color:"#aaaaaa",marginBottom:"8px"}}>✓ {ytVideos.length} live YouTube episodes ready</div>}
      {!data&&!loading&&<div style={{fontSize:"14px",color:"#aaaaaa",fontStyle:"italic"}}>Click "Generate with AI" for this week's recommendations.</div>}
...
function Sentiment({title, showName, views, color}) {
  ...
  return (
    <div>
      <button onClick={run} disabled={l} style={{background:"transparent",border:"1px solid #333",color:s?color:"#888888",padding:"4px 10px",fontSize:"14px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",borderRadius:"2px"}}>
      ...
      {open&&s&&(
        <div style={{marginTop:"8px",background:"#ffffff",border:"1px solid #222",borderRadius:"2px",padding:"12px 14px"}}>
          ...
          <button onClick={()=>setOpen(false)} style={{marginTop:"8px",background:"transparent",border:"none",color:"#aaaaaa",fontSize:"14px",cursor:"pointer",fontFamily:"'Barlow',sans-serif"}}>close ↑</button>
        </div>
      )}
    </div>
  );
...
function Insights({show, ytVideos}) {
  ...
  const Block=({title,type,result})=>(
    <div style={{background:"#ffffff",border:"1px solid #222",borderRadius:"2px",padding:"16px 18px",marginBottom:"10px"}}>
      ...
      {!result&&!loading[type]&&<div style={{fontSize:"14px",color:"#aaaaaa",fontStyle:"italic"}}>{hasYT?`Using ${ytVideos.length} live YouTube episodes.`:"Using spreadsheet data."}</div>}
      {loading[type]&&<div style={{fontSize:"14px",color:"#aaaaaa"}}>Analyzing…</div>}
        ...
        <div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginBottom:"12px"}}>
            {[
              ...
              {label:"Guest vs solo",val:result.delta||"N/A",accent:result.delta?.includes("+")?"#aaaaaa":"#de6f3f"},
            ].map((m,i)=>(
              <div key={i} style={{background:"#f5f5f5",borderRadius:"2px",padding:"10px 12px"}}>
                <div style={{fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",letterSpacing:".08em",marginBottom:"4px"}}>{m.label}</div>
                <div style={{fontSize:"16px",fontWeight:"500",color:m.accent||"#fff",fontFamily:"'Barlow',sans-serif"}}>{m.val}</div>
              </div>
            ))}
          </div>
          {result.insight&&<div style={{fontSize:"14px",color:"#666666",lineHeight:"1.65",marginBottom:"10px",paddingLeft:"10px",borderLeft:`2px solid ${color}44`}}>{result.insight}</div>}
          <div style={{display:"flex",gap:"8px",padding:"4px 0",marginBottom:"4px"}}>
            <div style={{flex:1,fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase"}}>Guest</div>
            <div style={{width:"200px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase"}}>Episode</div>
            <div style={{width:"70px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"right"}}>{result.metric||"Views"}</div>
          </div>
      ...
      {result&&type==="topics"&&(
        <div>
          {result.insight&&<div style={{fontSize:"14px",color:"#666666",lineHeight:"1.65",marginBottom:"10px",paddingLeft:"10px",borderLeft:`2px solid ${color}44`}}>{result.insight}</div>}
          <div style={{display:"flex",gap:"8px",padding:"4px 10px",marginBottom:"4px",background:"#f5f5f5",borderRadius:"2px"}}>
            <div style={{flex:1,fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase"}}>Topic</div>
            <div style={{width:"40px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"center"}}>Eps</div>
            <div style={{width:"65px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"right"}}>Avg {result.metric||"Views"}</div>
            <div style={{width:"55px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"right"}}>vs Avg</div>
          </div>
      ...
      {result&&type==="titles"&&(
        <div>
          {result.insight&&<div style={{fontSize:"14px",color:"#666666",lineHeight:"1.65",marginBottom:"10px",paddingLeft:"10px",borderLeft:`2px solid ${color}44`}}>{result.insight}</div>}
          <div style={{display:"flex",gap:"8px",padding:"4px 10px",marginBottom:"4px",background:"#f5f5f5",borderRadius:"2px"}}>
            <div style={{flex:1,fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase"}}>Pattern</div>
            <div style={{width:"80px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"right"}}>Avg {result.metric||"Views"}</div>
            <div style={{width:"55px",fontSize:"12px",color:"#aaaaaa",textTransform:"uppercase",textAlign:"right"}}>vs Avg</div>
          </div>
          {result.patterns?.map((p,i)=>(
            <div key={i} style={{marginBottom:"8px",paddingBottom:"8px",borderBottom:"1px solid #1a1a1a"}}>
              <div style={{display:"flex",alignItems:"center",gap:"8px",padding:"0 10px"}}>
                <span style={{flex:1,fontSize:"14px",color:"#1a1a1a",fontWeight:"500"}}>{p.pattern}</span>
                <span style={{width:"80px",fontSize:"14px",color:"#666666",textAlign:"right"}}>{fmt(p.avgViews||p.avgD7)} · {p.count} eps</span>
                <span style={{width:"55px",fontSize:"14px",color:p.vsBaseline?.includes("+")?"#aaaaaa":"#de6f3f",fontWeight:"500",textAlign:"right"}}>{p.vsBaseline}</span>
              </div>
              {p.examples?.[0]&&<div style={{fontSize:"12px",color:"#aaaaaa",marginTop:"3px",padding:"0 10px"}}>e.g. "{p.examples[0].slice(0,55)}"</div>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{marginTop:"20px"}}>
      <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".12em",textTransform:"uppercase",marginBottom:"12px"}}>Deep Insights{hasYT?` · using ${ytVideos.length} live YouTube episodes`:" · loading YouTube data…"}</div>
    ...
    <div>
      <div style={{marginBottom:"24px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"6px"}}>
          ...
          <h2 style={{fontSize:"22px",fontWeight:"700",color:"#fff",fontFamily:"'Barlow',sans-serif",margin:0}}>{name}</h2>
        </div>
        <div style={{fontSize:"14px",color:ytLoading?"#aaaaaa":"#aaaaaa",marginLeft:"16px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"10px",marginBottom:"20px"}}>
        {[
          {label:"Rolling 4-ep avg",val:fmt(r4avg),sub:p4avg?`${trend>=0?"+":""}${trend}% vs prior 4`:null,subColor:trend>=0?"#aaaaaa":"#de6f3f"},
          ...
          <div key={i} style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"16px 18px"}}>
            <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".1em",textTransform:"uppercase",marginBottom:"8px"}}>{m.label}</div>
            <div style={{fontSize:"22px",fontWeight:"500",color:m.accent||"#fff",fontFamily:"'Barlow',sans-serif"}}>{m.val}</div>
            {m.sub&&<div style={{fontSize:"14px",color:m.subColor||"#aaaaaa",marginTop:"4px"}}>{m.sub}</div>}
          </div>
        ))}
      </div>
      ...
      <div style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"20px 22px",marginBottom:"10px"}}>
        <div style={{display:"flex",gap:"8px",marginBottom:"16px"}}>
          {[["views","By Views"],["date","By Date"]].map(([k,l])=>(
            <button key={k} onClick={()=>setSort(k)} style={{background:sort===k?color:"transparent",border:`1px solid ${sort===k?color:"#bbbbbb"}`,color:sort===k?"#fff":"#aaaaaa",padding:"4px 12px",fontSize:"14px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",borderRadius:"2px"}}>{l}</button>
          ))}
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:"14px",fontFamily:"'Barlow',sans-serif"}}>
            <thead>
              <tr style={{borderBottom:"1px solid #222"}}>
                {["Date","Episode","Views","Likes","Comments","Guest","Sentiment"].map(h=>(
                  <th key={h} style={{padding:"8px 10px",textAlign:"left",color:"#aaaaaa",fontSize:"12px",letterSpacing:".1em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
  ...
  return (
    <div>
      <div style={{marginBottom:"28px"}}>
        <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".15em",textTransform:"uppercase",marginBottom:"4px"}}>{today}</div>
        <h1 style={{fontSize:"28px",fontWeight:"900",color:"#fff",fontFamily:"'Barlow',sans-serif",margin:"0 0 6px"}}>Weekly Snapshot</h1>
        <div style={{fontSize:"14px",color:loaded?"#aaaaaa":"#888888"}}>{loaded?"✓ Live YouTube data loaded for all shows":"Loading live YouTube data…"}</div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px",marginBottom:"28px"}}>
        {shows.map(s=>{
          ...
          return (
            <div key={s.id} style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"20px"}}>
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"14px"}}>
                ...
                {vids.length>0&&<span style={{fontSize:"12px",color:"#aaaaaa"}}>live</span>}
              </div>
              <div style={{fontSize:"24px",fontWeight:"700",color:"#fff",fontFamily:"'Barlow',sans-serif",marginBottom:"2px"}}>{fmt(r4)}</div>
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"12px"}}>
                <div style={{fontSize:"14px",color:"#aaaaaa"}}>rolling 4-ep avg views</div>
                {p4>0&&<div style={{fontSize:"14px",fontWeight:"500",color:trend>=0?"#aaaaaa":"#de6f3f"}}>{trend>=0?"+":""}{trend}% vs prior</div>}
              </div>
              <div style={{fontSize:"14px",color:"#1a1a1a",marginBottom:"4px"}}>{top?.title?.slice(0,55)}{(top?.title?.length||0)>55?"…":""}</div>
              <div style={{fontSize:"14px",color:"#aaaaaa"}}>{fmt(top?.views)} views · top episode</div>
            </div>
          );
        })}
      </div>

      <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".12em",textTransform:"uppercase",marginBottom:"14px"}}>AI Recommendations</div>
      {shows.map(s=>(
...
function Trends() {
  ...
  return (
    <div>
      <div style={{marginBottom:"24px"}}>
        <h2 style={{fontSize:"22px",fontWeight:"700",color:"#fff",fontFamily:"'Barlow',sans-serif",marginBottom:"6px"}}>Trends</h2>
        <div style={{fontSize:"14px",color:"#aaaaaa"}}>Historical performance by show</div>
      </div>
      <div style={{display:"flex",gap:"8px",marginBottom:"24px"}}>
        {shows.map(s=>(
          <button key={s.id} onClick={()=>setActive(s.id)} style={{background:active===s.id?s.color:"transparent",border:`1px solid ${active===s.id?s.color:"#bbbbbb"}`,color:active===s.id?"#fff":s.color,padding:"6px 16px",fontSize:"14px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",borderRadius:"2px"}}>{s.name}</button>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px",marginBottom:"20px"}}>
        {[
          ...
          <div key={i} style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"16px 18px"}}>
            <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".1em",textTransform:"uppercase",marginBottom:"8px"}}>{m.label}</div>
            <div style={{fontSize:"22px",fontWeight:"500",color:show.color,fontFamily:"'Barlow',sans-serif"}}>{m.val}</div>
            <div style={{fontSize:"14px",color:"#aaaaaa",marginTop:"4px"}}>{m.sub}</div>
          </div>
        ))}
      </div>

      {mavgs.length>0&&(
        <div style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"20px 24px",marginBottom:"14px"}}>
          <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".12em",textTransform:"uppercase",marginBottom:"16px"}}>Monthly avg views</div>
            ...
            {mavgs.map((m,i)=>{
              ...
              return (
                <div key={m.key} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",minWidth:0}}>
                  <div style={{fontSize:"8px",color:isRecent?show.color:"#aaaaaa",whiteSpace:"nowrap"}}>{fmt(m.avg)}</div>
                  <div style={{width:"100%",height:`${h}px`,background:show.color,opacity:isRecent?1:.4,borderRadius:"2px 2px 0 0"}}/>
                  <div style={{fontSize:"8px",color:"#bbbbbb",transform:"rotate(-45deg)",transformOrigin:"center",marginTop:"4px",whiteSpace:"nowrap"}}>{m.key.split("-")[1]+"/"+m.key.split("-")[0].slice(-2)}</div>
                </div>
              );
            })}
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}>
        <div style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"20px 22px"}}>
          <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".12em",textTransform:"uppercase",marginBottom:"14px"}}>Top 10 episodes</div>
          {top10.map((ep,i)=>(
        ...
        <div style={{background:"#f5f5f5",border:"1px solid #222",borderRadius:"2px",padding:"20px 22px"}}>
          <div style={{fontSize:"14px",color:"#aaaaaa",letterSpacing:".12em",textTransform:"uppercase",marginBottom:"14px"}}>Month by month</div>
    ...
    <div style={{minHeight:"100vh",background:"#ffffff",fontFamily:"'Barlow',sans-serif",color:"#00222d"}}>
      ...
      <div style={{position:"fixed",top:0,left:0,width:"200px",height:"100vh",background:"#ffffff",borderRight:"1px solid #1a1a1a",padding:"28px 0",display:"flex",flexDirection:"column",zIndex:10}}>
        ...
        {nav.map(n=>{
          ...
          return <button key={n.id} onClick={()=>setPage(n.id)} style={{background:"transparent",border:"none",textAlign:"left",padding:"9px 20px",fontSize:"14px",color:active?(show?.color||"#fff"):"#aaaaaa",cursor:"pointer",fontFamily:"'Barlow',sans-serif",letterSpacing:".04em",borderLeft:`2px solid ${active?(show?.color||"#de6f3f"):"transparent"}`,width:"100%"}}>{n.label}</button>;
        })}
        <div style={{marginTop:"auto",padding:"20px 20px 0",borderTop:"1px solid #1a1a1a"}}>
          <div style={{fontSize:"12px",color:"#bbbbbb",lineHeight:"1.6"}}>profg2025<br/>profg-dashboard.vercel.app</div>
        </div>
      </div>
