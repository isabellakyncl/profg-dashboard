...
// ── Brand ─────────────────────────────────────────────────────────────────────
const B = {bg:"#00222d",card:"#00303f",cardAlt:"#003a4a",border:"#005066",borderSub:"#003d52",text:"#ffffff",textSub:"#b0d4db",textMute:"#6a9aa3",teal:"#4fafb8",yellow:"#f4b547",orange:"#de6f3f",pink:"#d987b5",green:"#4fafb8",red:"#de6f3f",font:"'Barlow', sans-serif"}
    ...
    <div>
      ...
      {eps.length>1&&(
        <div style={{...card,marginBottom:"14px"}}>
          <div style={{...label,marginBottom:"16px"}}>YouTube views per episode — last {eps.length} episodes, oldest to newest (click dot for details)</div>
      ...
      {eps.length>1&&(
        <div style={{...card,marginBottom:"14px"}}>
          ...
          {(()=>{
            ...
            return (
              <div style={{width:"100%"}}>
                <div style={{position:"relative",height:"160px",width:"100%",marginBottom:"8px"}}>
                  {points.map((p,i)=>{
                    const left = `${p.x}%`;
                    const bottom = `${100 - (p.y/120*100)}%`;
                    return (
                      <div key={i} onClick={()=>setSelEp(selEp===i?null:i)}
                        style={{
                          position:"absolute",
                          left:left,
                          bottom:bottom,
                          ...
                          height:"12px",
                          borderRadius:"50%",
                          background:selEp===i?B.yellow:show.color,
                          cursor:"pointer",
                          zIndex:2,
                  ...
                  <svg style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",pointerEvents:"none"}}>
                    <polyline
                      points={points.map(p=>`${p.x}%,${p.y/120*100}%`).join(" ")}
                      fill="none" stroke={show.color} strokeWidth="2" opacity="0.6" strokeLinejoin="round"/>
                  </svg>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:"12px"}}>
                  <div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font}}>{chartEps[0]?.date?.slice(0,10)}</div>
                  <div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font}}>click any dot for details</div>
                  <div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font}}>{chartEps[chartEps.length-1]?.date?.slice(0,10)}</div>
                </div>
                {selEp!==null&&points[selEp]&&(
                  <div style={{background:B.cardAlt,border:`2px solid ${show.color}`,borderRadius:"4px",padding:"16px 18px",marginTop:"4px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:"15px",fontWeight:"700",color:B.text,fontFamily:B.font,marginBottom:"10px"}}>{points[selEp].e.title}</div>
                        <div style={{display:"flex",gap:"24px",flexWrap:"wrap"}}>
                          <div><div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font,marginBottom:"2px"}}>YouTube views</div><div style={{fontSize:"20px",fontWeight:"900",color:show.color,fontFamily:B.font}}>{fmt(points[selEp].e.views)}</div></div>
                          {points[selEp].e.likes>0&&<div><div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font,marginBottom:"2px"}}>Likes</div><div style={{fontSize:"20px",fontWeight:"900",color:B.text,fontFamily:B.font}}>{fmt(points[selEp].e.likes)}</div></div>}
                          {points[selEp].e.comments>0&&<div><div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font,marginBottom:"2px"}}>Comments</div><div style={{fontSize:"20px",fontWeight:"900",color:B.text,fontFamily:B.font}}>{fmt(points[selEp].e.comments)}</div></div>}
                          <div><div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font,marginBottom:"2px"}}>Published</div><div style={{fontSize:"14px",fontWeight:"600",color:B.text,fontFamily:B.font}}>{points[selEp].e.date?.slice(0,10)}</div></div>
                          {points[selEp].e.guest&&<div><div style={{fontSize:"11px",color:B.textMute,fontFamily:B.font,marginBottom:"2px"}}>Guest</div><div style={{fontSize:"14px",fontWeight:"700",color:show.color,fontFamily:B.font}}>{points[selEp].e.guest}</div></div>}
                        </div>
                      </div>
                      <button onClick={()=>setSelEp(null)} style={{background:"transparent",border:"none",color:B.textMute,fontSize:"22px",cursor:"pointer",fontFamily:B.font,lineHeight:1,marginLeft:"12px"}}>×</button>
                    </div>
                  </div>
                )}
              </div>
